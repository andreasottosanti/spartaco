...
We will use the `SpatialExperiment` object; however, the same analysis can be done using the the object `sce`. Not however that the functions into the `spartaco` package are not designed to deal with `SingleCellExperiment` objects, so the experiment matrix and the spatial coordinates must be manually passed to the estimation function. We concentrate on the sample with ID 151673:
...
problem. We begin selecting only those whose number of counts is at least 300.
...
SpaRTaCo can be run for a given number of iterations starting from different sets of initial points. Since `spe` is an object of class `SpatialExperiment`, it can be used directly into the estimation function. To fit the model, use the function

```{r, eval = F}
results <- spartaco_multirun(spe, assay = "binomial_deviance_residuals", K = 2, R = 9, nstart = 5, max.iter = 3000, mc.cores = 5)
```
 
In this case, the applied model searches for 18 blocks (2 row clusters and 9 column clusters), and the estimation algorithm is run for 3,000 iterations starting from 5 different sets of points.
