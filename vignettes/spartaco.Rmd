...
In this vignette, we show how to use  the functions of the R package `spartaco` to fit the SpaRTaCo model (*Sottosanti and Risso, 2021+*) on 10X Visium spatial experiments. We consider one of the human dorsolateral prefrontal cortex tissue samples contained into the `R` package [spatialLIBD](http://spatial.libd.org/spatialLIBD/) (click the link to have more details about the data). The following analysis is extracted from  Sottosanti and Risso's article. Note that, due to the high computational costs that the estimation of SpaRTaCo requires, most of the computations have  been run locally, and the results uploaded.
...
vis_clus(spe = spe, sampleid = "151673", clustervar = "layer_guess_reordered", colors = libd_layer_colors,
...
In the following figure, we display the boxplots of the first 100 genes transformed according to the binomial and the Poisson deviance residuals.
...
SpaRTaCo can be fitted on the deviance residuals obtained at the previous step. The estimation function is run for a given number of iterations starting from different sets of initial points. The code is the following:

```{r, eval = F}
results <- spartaco_multirun(data = spe, assay = "binomial_deviance_residuals", K = 2, R = 9, nstart = 5, max.iter = 3000, mc.cores = 5)
```
 
In this case, the estimated SpaRTaCo has 18 blocks (2 row clusters and 9 column clusters), and the estimation algorithm is run for 3,000 iterations starting from 5 different sets of points. It is possible to set the number of cores to be employed into the process through `mc.cores`. Note that `spe` is an object of class `SpatialExperiment` and can be used directly into the `spartaco` function. In alternative, the following code line is equivalent to the previous one.

```{r, eval = F}
x <- as.matrix(spe@assays@data$binomial_deviance_residuals)
coordinates <- spatialCoords(spe)
results <- spartaco_multirun(data = x, coordinates = coordinates, K = 2, R = 9, nstart = 5, max.iter = 3000, mc.cores = 5)
```

To run SpaRTaCo simultaneously with multiple dimensions, one could use for example the function `parallel::mclapply`. Let us suppose that we evaluate from 7 to 12 column clusters. The code to perform this operation is
...
ggplot(data.frame(R.values, ICL.values), aes(R.values, ICL.values/1e+6))+geom_point()+theme_classic()+ylab("ICL (1e+6)")
```
...
```{r, eval = T, fig.width = 6.5, fig.height=4, fig.align="center"}
...
```{r, eval = T, fig.width = 6.5, fig.height=5, fig.align="center"}
